    ...
    def on_train_batch_end(
        ...
        if pl_module.global_step % self.log_every_n_steps == 0:
            ...
            with torch.no_grad():
                ...
                #images: Dict[str, torch.Tensor] = pl_module.log_images(batch, **self.log_images_kwargs)   #从这里调用的log_images!!
                #images, mixer, idx_path, img_path, img_path_label = pl_module.log_images(batch, **self.log_images_kwargs)
            ...
            for image_key in images: #images
                ...
                # if image_key == 'pred':
                #     filename = "{}_step-{:06}_e-{:06}_b-{:06}_mixer-{}_path-{}_gate1pred-{}_gate1true-{}.png".format(
                #         image_key, pl_module.global_step, pl_module.current_epoch, batch_idx, mixer, idx_path, img_path, img_path_label
                #     )
                if image_key == 'pred':
                ...
                # filename = "{}_step-{:06}_e-{:06}_b-{:06}.nii.gz".format(
                #     image_key, pl_module.global_step, pl_module.current_epoch, batch_idx
                # )
                # path = os.path.join(save_dir, filename)
                # sitk.WriteImage(grid, path)
